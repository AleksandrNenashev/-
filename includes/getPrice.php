	...
	if(!empty($parent_id))
	{
		...
		if (!is_array($ret)){
				...
				while($ob = $res->GetNextElement())
				{
                    ...
                    //$el_arr[$arFields['ID']]['test'] = $arFields;
				}
                ...
                if(is_array($parent_id)) {
                    foreach ($parent_id as $prod_id){
                        if (CCatalogProductSet::isProductInSet($prod_id, CCatalogProductSet::TYPE_SET)){
                            $ar_res = CPrice::GetBasePrice($prod_id);
                            $arDiscounts = CCatalogDiscount::GetDiscountByProduct(
                                $prod_id,
                                $USER->GetUserGroupArray(),
                                "N",
                            ...
                            $ret[$prod_id]['PRINT_VALUE_NOVAT'] = number_format($ar_res['PRICE'], 0, ',', ' ') . ' Р';
                            $disc_prod = array_shift($arDiscounts)['VALUE'];
                            $ret[$prod_id]['PRINT_DISCOUNT_VALUE'] = number_format((1 - $disc_prod/ 100) * $ar_res['PRICE'], 0, ',', ' ') . ' Р';
                            $ret[$prod_id]['DISCOUNT_DIFF_PERCENT'] = $disc_prod;
                            $ret[$prod_id]['ONE'] = $prod_id;
                        }
                    }
                }
