<?
function getPriceNoSKU($parent_id){
// $parent_id - здесь ID самих товаров
	...
	if(!empty($parent_id))
	{
		...
		$cache = new CPHPCache();
		$cache_time = 3600;
		$cache_id = 'getPrice_cache_reg2-'.$_SESSION['region_price_id'].'_id-'.$cache_ind;
		...
		if (!is_array($ret)){
				...
				$arSelect = Array("ID", "NAME", "CATALOG_GROUP_".$_SESSION['region_price_id'], "CATALOG_PRICE_".$_SESSION['region_price_id']);
				$arFilter = Array("IBLOCK_ID"=>1, "ACTIVE"=>"Y", "ID" => $parent_id);
				...
				while($ob = $res->GetNextElement())
				{
					...
					if(!empty($discountPrice)){
						$with_discount[$arFields['ID']] = $discountPrice;
					}
				...
				$ret = $el_arr;
/* пока непонятно, как это нужно в товарах без sku
